...
- `GET /scenarios` — список только опубликованных и уже открытых игрокам сюжетных линий; каждый элемент содержит `max_score` для честного отображения статусов прогресса
- `GET /scenarios/{slug}` — детали live-сценария и шаги

## Sessions

- `POST /sessions` — старт новой миссии; выключенные, запланированные и уже идеально завершённые сценарии не запускаются повторно
